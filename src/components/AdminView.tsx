...
export default function AdminView({
  ...
  // 1. useEffect ดึงข้อมูลจริงทั้งหมดเมื่อโหลดหน้าแอดมิน
  useEffect(() => {
    const fetchRealData = async () => {
      // ดึงเมนูสินค้า (Products)
      ...
      // ดึงออเดอร์ (Orders)
      const { data: realOrders, error: oError } = await supabase
        .from('orders')
        .select('*')
    ...
    // ตั้งค่า Realtime อัปเดตออเดอร์
  ...
  }, [setProducts, setOrders]); // ส่งฟังก์ชัน set เข้าไปใน dependency array ด้วย
  ...
  // 2. คำนวณเมนูขายดีประจำสัปดาห์จากข้อมูลออเดอร์จริง
  const topSellers = useMemo(() => {
    ...
    // เรียงลำดับจากขายดีสุดไปน้อยสุด
        ...
        {activeAdminTab === 'dashboard' && (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Best Seller Card (เชื่อมข้อมูลจริง) */}
              <div className="bg-[#feeae0] p-4.5 rounded-2xl border border-[#ddc1b3]/30 flex flex-col justify-between shadow-sm">
                <div>
                  ...
                  <h4 className="text-base font-extrabold text-[#231914] mt-1 leading-tight line-clamp-1">
                    {topSellers.length > 0 ? topSellers[0].name : "ยังไม่มีข้อมูล"}
                  </h4>
                </div>
            ...
            {/* Top Selling item bar indicators (เชื่อมข้อมูลจริง) */}
              ...
              <div className="flex flex-col gap-3">
                {topSellers.length > 0 ? (
                  topSellers.slice(0, 3).map((item, index) => {
                    const maxSold = topSellers[0].count; // นำอันดับ 1 มาเป็นเกณฑ์ 100%
                    const widthPercent = Math.max((item.count / maxSold) * 100, 10); // ไม่ให้แถบสั้นเกินไป
                    const colors = ["bg-[#ff8c42]", "bg-[#ff8c42]/80", "bg-[#9b4500]/70"];

                    return (
                      <div key={index} className="flex flex-col gap-1">
                        <div className="flex justify-between text-xs font-semibold">
                          <span className="text-[#564338]">{item.name}</span>
                          <span className="text-[#9b4500]">{item.count} ชิ้น</span>
                        </div>
                        <div className="w-full bg-[#f2dfd5]/40 h-2 rounded-full overflow-hidden">
                          <div 
                            className={`${colors[index] || "bg-stone-300"} h-full rounded-full transition-all duration-500`} 
                          ...
                          <label id={`btn-upload-photo-label-${order.id}`} className="cursor-pointer bg-[#9b4500]/10 hover:bg-[#9b4500]/20 text-[#9b4500] text-[10px] font-bold px-2.5 py-1 rounded-full transition-all duration-150 active:scale-95 flex items-center gap-1">
                            ...
                            <input 
                              ...
                              onChange={(e) => {
                                const file = e.target.files?.[0];
                                if (file) {
                                  handleUpdateOrderPhoto(order.id, file);
                                }
                              }}
                            />
                          </label>
              ...
              {isAddingProduct && (
                ...
                >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {newProductImage ? (
                        ...
                        <label className="cursor-pointer bg-[#fff8f6] hover:bg-[#feeae0]/40 text-[#9b4500] border border-[#ddc1b3]/50 border-dashed rounded-xl h-[38px] px-3.5 flex items-center justify-center gap-1.5 transition-all text-xs font-bold">
                          ...
                          <span>{isUploadingImg ? 'กำลังอัปโหลด...' : 'เลือกรูป/ถ่ายรูป'}</span>
                          <input 
                            type="file" 
                            accept="image/*" 
                            className="hidden" 
                  ...
                  <button 
                    type="submit"
                    disabled={isUploadingImg}
                    className="w-full mt-2 bg-[#9b4500] hover:bg-[#ff8c42] disabled:bg-stone-300 text-white py-2.5 rounded-full font-bold text-xs shadow-sm transition-all duration-150 active:scale-98"
                  >
